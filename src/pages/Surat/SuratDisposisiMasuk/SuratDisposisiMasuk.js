  ...
  useEffect(()=>{
    if(dataDisposisi.data) {
      ...
      const filteredData = dataDisposisi.data.filter(item => {
        let mappedComponent = ['perihal','catatan','nama', 'tanggal']
        ...
        if (!searchComponent || searchComponent.length === 0) {
          ...
          flags.nama = item.nama_pengirim.toLowerCase().includes(searchTerm.toLowerCase())
          flags.catatan = item.isi_note.toLowerCase().includes(searchTerm.toLowerCase())
        }else {
          ...
          if(mappedComponent.includes("catatan")){
            flags.catatan = item.isi_note.toLowerCase().includes(searchTerm.toLowerCase())
          }
                  ...
                  >
                    <Select
                      ...
                      options={[
                        ...
                        { value: 'nama', label: 'Pengirim' },
                        { value: 'catatan', label: 'Isi Catatatan' },
